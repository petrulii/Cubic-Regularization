...
class CubicRegularization(Algorithm):
    ...
    def cubic_reg(self):
        ...
        while iter < self.maxiter and converged is False:
            print("grad_x?", self.grad_x)
            print("hess_x?", self.hess_x)
            x_old = x_new.copy()
...
class _AuxiliaryProblem:
    ...
    def solve(self):
        ...
        if self.method == "trust_region":
            ...
            (v, r) = newton(f, x0, args=(eta, eigvals, self.M), maxiter=200, full_output=true, tol=1.48e-05)
            if self.verbose == 1:
            ...
            s = np.matmul(O.T, u)
            print("grad_x shape", self.grad_x.shape)
            print("grad_x", self.grad_x)
